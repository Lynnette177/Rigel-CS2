...
void Cheats::Menu()
{
	...
	ImGui::Begin("ChumChum Counter Strike 2", nullptr, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoCollapse);
	{
		...
		if (ImGui::BeginTabItem("ESP"))
		{
			...
			Gui.MyCheckBox("GlowESP", &MenuConfig::ShowGlow);
		...
		if (ImGui::BeginTabItem("AimBot "))
		{
			...
			if (ImGui::Combo("AimKey", &MenuConfig::AimBotHotKey, "LBUTTON\0MENU\0RBUTTON\0XBUTTON1\0XBUTTON2\0CAPITAL\0SHIFT\0CONTROL"))
			{
				AimControl::SetHotKey(MenuConfig::AimBotHotKey);
			}

			float FovMin = 0.1f, FovMax = 89.f;
			float SmoothMin = 0.01f, SmoothMax = 1.5f;
			...
			Gui.SliderScalarEx1("Speed", ImGuiDataType_Float, &AimControl::Smooth, &SmoothMin, &SmoothMax, "%.2f", ImGuiSliderFlags_None);
		...
		ImGui::SameLine();
		Gui.MyCheckBox("ESP ON when Spec", &MenuConfig::ShowWhenSpec);
...
void Cheats::RadarSetting(Base_Radar& Radar)
{
	...
	// Radar window
	ImGui::Begin("ChumChumRadar", 0, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar);
...
void Cheats::Run()
{
	...
	/*	for (int BoneIndex = 0; BoneIndex < Entity.BoneData.BonePosList.size(); BoneIndex++)
		...
		if (MenuConfig::ShowBoneESP) {
			if (MenuConfig::VisibleCheck && Entity.Pawn.bSpottedByMask & (DWORD64(1) << (LocalPlayerControllerIndex)) ||
				LocalEntity.Pawn.bSpottedByMask & (DWORD64(1) << (i)))
		...
		float glow1 = 0;
		if (MenuConfig::ShowGlow) {
			glow1 = 86400.f;
		}
		else
			glow1 = 0.f;
		ProcessMgr.WriteMemory<float>(Entity.Pawn.Address + 0x5060, glow1);//
		// Draw eyeRay
